...
Возврат по ссылке происходит реже, при этом важно следить за тем, что объект на который возвращается ссылка должен жить и после выхода из функции. Обычно так возвращаются параметры функции, которые мы приняли по ссылке. Также ярким примером является `operator[]` например для вектора.
