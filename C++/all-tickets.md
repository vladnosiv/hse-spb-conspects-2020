...
* Для начала разберемся, зачем это нужно, поймем, что мы это используем часто и постараемся не думать о том, как это работает.
    ...
    Но вот если не задали... Тогда компилятор должен угадать, что же вы имели ввиду. Например, безобидная конструкция ```std::cout << "Hello world!";``` это уже ADL. Потому что это на самом деле вызов ```std::operator<<(std::ostream&, T)```, но мы нигде не подсказали компилятору, где искать такой оператор, поэтому он отправляется на увлекательные поискы оператора, что само по себе крайне полезно, если не задумываться о том, как он это делает.
...
* Что же, вы все еще это читаете, значит, ждете чего-нибудь страшного? Тогда давайте разберем этот крайне занимательный код.
    ...
    int main() {
        ...
        // а ADL не брезгливый, он возьмет даже у std.
...
* Пользоваться ADL можно, но могут возникать казусы. В коде ```count(vec.begin(), vec.end())``` аргументы это:
