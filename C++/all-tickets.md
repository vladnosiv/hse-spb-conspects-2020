...
* Порядок инициализации/уничтожения подобъектов и полей, как передать параметры конструкторам
	...
	* Сначала инициализируются базовые объекты, причем в том порядке, в котором они указаны в объявлении класса (получается своеобразный dfs, т.к. заходим в первый базовый класс, у него заходим в первый базовый класс и т.д.). После этого как обычно (в порядке объявлния) инициализируются поля. Параметры конструкторам базовых классов передаются через запятую.
...
Как понятно, пока объект живёт - инвариант выполняется. Отсюда вытекают некоторые условия:

	Постусловие конструктора - инвариант выполняется
	Предусловие конструктора - поля сконструированны
	Постусловие деструктора - объект корректно уничтожен
	Предусловие деструктора - инвариант выполняется
 ...
 * Возвращать код ошибки
   ...
   ```
   Проблема конкретно этого подхода сразу видна - надо знать все значения кода, что не очень удобно для программиста
 * Почти возвращение кода ошибки - возвращать структуру, в которой может лежать значение OK или Error
   ...
   *пример на Rust'е*
   ```Rust
     ...
     let f = try!(open());
   ```
   Здесь Result - как раз такая структура. Если ошибок нет, она вернёт значение OK или значение Err в противном случае.
 ...
 * В `catch` иногда полезно что-то сделать, а затем прокинуть ошибку дальше (например, если мы хотим проследить жизненный путь ошибки). Осуществляется конструкцией `throw;`
...
Ловят исключение по ссылке, потому что очень активно используется иерархия исключений - хотим избежать slicing'а.
### Stack unwinding (раскрутка стека) и уничтожение локальных ресурсов
...
* Также исключения могут вылетать из member initializer list (поля, базовые классы). Тогда не совсем ясно, что можно с этим вообще сделать. Конечно, существует такой синтаксис, называется **function try block**:
    ...
    _Замечание_: в конце такого блока `catch` всегда неявно будет дописываться `throw` (только в конструкторах).

* В отдельный случай можно выделить **делегирующие конструкторы**:
    ```c++
    struct Foo {
        Foo(int x) {
...
Непойманные исключения - аварийно завершают программу, при это возможно такое, что программа поймет, что исключение никто не поймает, и завершится **без** раскрутки стека.
...
### Объект-функтор
Функтором называется класс (или структура) с перегруженным `operator()`. Могут применяться также, как и функции, однако имеют преимущества. Например, хотим отсортировать массив по компаратору `a[i] * x`, где `x` — константа. Для обычной функции-компаратора пришлось бы заводить отдельную глобальную константу, что не очень хорошо. Однако в функторе мы можем хранить `x`, как поле класса. Возможная реализация:
```cpp
struct Comparator {
    int x;
...
### Лямбды
Все это, конечно, очень круто, но иногда нам нужно использовать функтор всего в одном месте, и как-то не хочется для этого создавать отдельный класс. Или нам хочется создать функцию внутри другой функции. Для этого придумали лямбды. Синтаксис такой:
```cpp
...
Также, компилятор все оптимизирует на этапе компиляции.
...
Проблема в том, что вызов метода у `std::function` — это виртуальный вызов и он не всегда будет соптимизирован. В то же время лямбда не имеет виртуальных вызовов. Из-за этого `std::function` может работать сильно медленнее обычной лямбды.
...
* ### Типичная реализация конструктора с копированиями
  ...
  * Простой пример
  ...
  ```
  Наш конструктор принимает копию какой-то строки, после выхода из функции эта копия исчезнет, и никем не будет использоваться, поэтому имеет смысл сделать move из нее, чтобы избежать лишних копироваий. `std::move` здесь по делу, иначе произойдет копирование.

  * Подобные вещи могут происходить и в других методах класса (почему-то в интернете есть примеры лишь на конструктор, но то же самое можно делать в set-методах, например).
...
* ### Правила привязывания ссылок
  ...
  * `Foo &&f = ..; // только rvalue`
  ...
  `const Foo &&f` может привязаться только к `rvalue`, при этом из-за константности из нее нельзя сделать move, который мы обычно применяем к rvalue-ссылкам. Поэтому конструкция бесполезна, но есть какие-то специфические случаи, описанные [здесь](https://www.codesynthesis.com/~boris/blog/2012/07/24/const-rvalue-references/).
...
* Обычно должны быть операторы присваивания (копирования, перемещения)
...
Язык С++ позволяет использовать всю стандартную библиотеку языка С, но появляется еще и собственная стандартная библиотека. Заметим, что STL (standard template library) - кусок стандартной библиотеки языка C++. Конвенция в стандартной библиотеке C++: заголовок пишется без расширений (например, без `.h`). Добавил что-то свое в `std` - за парой исключений получил UB. Для стандартных заголовков из C есть два варианта их подключения:
...
* Итераторы не инвалидируются никогда. Удобно, правда? Но при этом если произойдёт `swap`, тогда то, на что указывали итераторы, может поменяться (хранилища местами поменяли). Что прикольно: **такое же** поведение и в других нижеперечисленных, **кроме** string
...
* `a.assign(begin, end)` — переприсвоить элементы на значения из полуинтервала (или контейнера, или сделать `n` копий). Умеет ещё из InputIterator-а брать.
  ...
  v.assign(d);
  ```
* `a.clear()` - уничтожает (вызывает деструкторы) всех объектов в контейнере. У вектора и строчки память не чистит! (до C++11 может и чистит, а после точно нет, `capacity` остаётся тем же). Про обращения с памятью у листа и дека на cppreference я не нашёл, возможно, это зависит от конкретной реализации. Написано лишь, что инвалидирует все итераторы, указатели и т.п.
...
* Есть встроенные методы, которые работают с индексами вместо итераторов.
  * `a.substr(first, length = std::string::npos)` // здесь std::string::npos - это максимально возможное значение `size_t`, то есть по сути замена `end`, возьмём подстроку до конца строки
...
В билете указано отдельно про `end()` - ну да, `begin(), end()` тоже могут инвалидироваться, например, когда делаешь`push_back()`, `push_front()` или `insert`. 
...
* **Default initialization**: `T t;` (в том числе поля), `new T`.
  ...
  **примечание от читающих ночью: возможно, здесь всё таки имелось в виду default member initializer?**
