...
Удаление по итератору работает амортизировано за `O(1)`.
Удаление по значению за `O(log n)`.

Для вставки можно использовать подсказки (метод `emplace_hint`). Если подсказка верна, то добавление работает амортизировано за `O(1)`.
...
Есть метод `at()`, который бросает исключение в случае, если элемента с таким ключом нет.

### `unordered_set`, `unordered_multiset`, `unordered_map`, `unordered_multimap`

Элементы лежат в порядке, который зависит от хеш-функции. 
Все операции с элементами по умолчанию работают за `O(1)` амортизировано.
